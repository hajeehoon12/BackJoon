...
while True:
    ...
    for i, height in enumerate(graph): # enumerate graph로 변경 (0, 숫자) 와 같은 형식
        ...
        if stack and stack[-1][1] > height:
            while stack:  # 스택에서 빼내며 최대 직사각형 넓이를 계산합니다.
                ...
                width_start = 1
                if stack:
                    width_start = stack[-1][0]+1  # 현재 너비
                result = (i - width_start) * stack_height # 현재 스택까지의 최대 넓이
                max_result = max(result, max_result) # 최대값 갱신
                # 스택에 들어있는 막대 중에서 현재 막대의 길이보다 큰 것들만 꺼내서 계산합니다. 만약 다음 스택이 조건 불만족시 break
    ...
    # 이전 반복문이 길이 차이로 인해서 종료되고, 스택에 남은 막대기가 있다는 가정하에서만의 계산식.
    while stack:
        stack_i, stack_height = stack.pop()
        width_start = 1
        if stack:
