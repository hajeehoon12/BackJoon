...
def preorder(in_start, in_end, post_start, post_end):
    ...
    root = position[parents]

    # left, right로 나눠 분할 정복 방식으로 트리를 추적하여 전위 순회를 찾아냄
    #preorder(in_start, root-1, post_start, post_start - in_start + root - 1) # 쪽 서브트리
    #preorder(root + 1, in_end, post_end-in_end + root, post_end-1) # 오른쪽 서브트리
    preorder(in_start, root-1, post_start, post_start - in_start - 1 + root)  # 쪽 서브트리
    preorder(root + 1, in_end, post_end-in_end + root, post_end-1) # 오른쪽 서브트리
